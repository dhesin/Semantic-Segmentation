...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    # TODO: Implement function
    fcn0 = tf.layers.conv2d(vgg_layer7_out, num_classes, 1, 1, padding='same', 
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
        
    fcn1 = tf.layers.conv2d_transpose(fcn0, num_classes, 4, strides=(2, 2), padding='same', 
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    vgg_layer_4_1by1 = tf.layers.conv2d(vgg_layer4_out, 1, 1, padding='same', 
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    fcn2 = tf.add(fcn1, vgg_layer_4_1by1)
    fcn3 = tf.layers.conv2d_transpose(fcn2, num_classes, 4, strides=(2, 2), padding='same', 
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    vgg_layer_3_1by1 = tf.layers.conv2d(vgg_layer3_out, 1, 1, padding='same', 
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    fcn4 = tf.add(fcn3, vgg_layer_3_1by1)
    fcn5 = tf.layers.conv2d_transpose(fcn4, num_classes, 16, strides=(8, 8), padding='same', 
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
...
def run():
    ...
    with tf.Session() as sess:
        ...
        prob_tensor = tf.placeholder(tf.float32)
        
        input_tensor, prob_tensor, layer3_tensor, layer4_tensor, layer7_tensor = load_vgg(sess, vgg_path)
        fcn_out = layers(layer3_tensor, layer4_tensor, layer7_tensor, num_classes)
